...
class DubaiPropertyForecaster:
    ...
    def _prepare_chronos_grid(self, df):
        """
        Prepare uniform monthly grid for Chronos forecasting, 
        safely handling index reset and duplicate column prevention.
        """
        if 'month' in df.columns:
            df['month'] = pd.to_datetime(df['month'])
            
        # Set index and resample monthly
        grid_df = df.set_index('month').groupby('model_area_id').resample('MS').asfreq()
        
        # Drop conflicting duplicate column if present before resetting index
        if 'model_area_id' in grid_df.columns:
            grid_df = grid_df.drop(columns=['model_area_id'], errors='ignore')
            
        grid_df = grid_df.reset_index()
        
        # Fallback safeguard against any remaining duplicate columns
