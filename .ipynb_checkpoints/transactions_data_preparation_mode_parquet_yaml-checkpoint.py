...
def process_and_merge_transactions(trans_path, micro_path, output_path, proj_grade_path, 
    ...
    if check_file_exists(bld_grade_path):
        cols_before = set(df.columns)
    ...
    # CURRENT CODE:
    if check_file_exists(dev_grade_path):
        cols_before = set(df.columns)
    ...
    if check_file_exists(proj_grade_path):
        cols_before = set(df.columns)
...
def run_merging_pipeline():
    ...
    def safe_s3_join(base, file_path):
        return file_path if str(file_path).startswith("s3://") else os.path.join(base, file_path)

    paths_dict = {
        'transactions': safe_s3_join(data_dir, trans_file),
        'micro_metadata': safe_s3_join(data_dir, micro_file),
        'projects_path': safe_s3_join(data_dir, proj_parquet),
        'developers_path': safe_s3_join(data_dir, dev_parquet),
        'proj_grade_path': safe_s3_join(data_dir, proj_grade_file),
        'dev_grade_path': safe_s3_join(data_dir, dev_grade_file),
        'bld_grade_path': safe_s3_join(data_dir, bld_grade_file),
        'output': safe_s3_join(data_dir, output_file_name)
    }
