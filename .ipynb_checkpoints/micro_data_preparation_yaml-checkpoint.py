...
def ensure_parent(path_str):
    ...
    if not str(path_str).startswith("s3://"):
        Path(path_str).parent.mkdir(parents=True, exist_ok=True)

@mlflow.trace(name="ingestion_project_data")
def get_processed_project_data(config):
    
    projects = pd.read_parquet(config['paths']['projects_file'])
    print("Projects columns:", projects.columns.tolist())
...
@mlflow.trace(name="ingestion_building_data")
...
def run_ingestion(config=None):
    ...
    # Data is already processed here in memory!
    ...
    # --- DIRECT MLFLOW LOGGING (NO EXTRA S3 DOWNLOADS NEEDED) ---
    # --- DIRECT MLFLOW LOGGING (FORCE ATTACH) ---
    # --- DIRECT MLFLOW LOGGING (ALREADY ACTIVE) ---
    import mlflow
    
    print("\n--- Logging to active MLflow run ---")
    
    unique_proj = int(pd_df['project_id'].nunique()) if 'project_id' in pd_df.columns else len(pd_df)
    unique_dev = int(pd_df['developer_id'].nunique()) if 'developer_id' in pd_df.columns else len(pd_df)
    
    # 1. Send Metrics (Automatically attaches to the run main.py started!)
    mlflow.log_metric("total_unique_projects", unique_proj)
    mlflow.log_metric("total_unique_developers", unique_dev)
    mlflow.log_metric("ingestion_processed_rows", len(final_df))
    
    # 2. Send JSON Artifacts
    proj_json = "latest_10_projects.json"
    dev_json = "latest_10_developers.json"
    
    pd_df.head(10).to_json(proj_json, orient="records", date_format="iso")
    mlflow.log_artifact(proj_json)
    
    if 'registration_date' in pd_df.columns:
        pd_df.sort_values(by='registration_date', ascending=False).head(10).to_json(dev_json, orient="records", date_format="iso")
    else:
        pd_df.head(10).to_json(dev_json, orient="records", date_format="iso")
    mlflow.log_artifact(dev_json)
    
    print("SUCCESS: Metrics and JSON files sent to MLflow!\n")
