...
def ensure_old_dir(base_dir, config):
    ...
    old_dir = folder if str(folder).startswith("s3://") else os.path.join(base_dir, folder)
...
def archive_existing_file(file_path, old_dir, prefix="old_"):
    # Skip local archiving if the file is in S3, as shutil.move only works locally
...
def _resolve_model_paths(config):
    ...
    def safe_s3_join(b, p):
        return p if str(p).startswith("s3://") else os.path.join(b, p)

    # Safely assign paths
    config['paths']['input_full'] = safe_s3_join(base, config['paths']['model_input'])
    config['paths']['m_dir'] = safe_s3_join(base, config['paths']['models_dir'])
    config['paths']['c_dir'] = safe_s3_join(base, config['paths']['columns_dir'])
    config['paths']['metrics_path'] = safe_s3_join(base, config['paths'].get('metrics_file', 'all_area_metrics.csv'))
    config['paths']['ranges_path'] = safe_s3_join(base, config['paths'].get('ranges_file', 'input_ranges.csv'))
    config['paths']['old_dir'] = safe_s3_join(base, config['paths'].get('old_files_dir', 'old_files'))
    
    # Only create local directories if they are NOT S3 paths
    for dir_key in ['m_dir', 'c_dir', 'old_dir']:
        dir_path = config['paths'][dir_key]
        if not str(dir_path).startswith("s3://"):
