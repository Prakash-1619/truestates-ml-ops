...
def execute_forecasting_tracking(config):
    """
    Tracks forecasting stage outputs and artifacts in MLflow:
    - Area-wise projection timelines and final forecast CSV files
    """
    ...
    if forecast_path.exists():
        ...
        print("\n" + "="*90)
        print(" FORECASTING STAGE PROJECTIONS SUMMARY ".center(90, "="))
        print("="*90)
    ...
    if stage in SCRIPT_MAP:
        ...
        with mlflow.start_run(run_name=f"stage_{stage}"):
            ...
            try:
                ...
                logger.error(f"Error logging forecasting metadata to MLflow: {e}")
    else:
