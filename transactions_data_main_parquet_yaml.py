...
def load_config(config_path="config.yaml"):
    ...
    config['paths']['trans_in'] = trans_in if str(trans_in).startswith("s3://") else os.path.join(base, trans_in)
    config['paths']['trans_out'] = trans_out if str(trans_out).startswith("s3://") else os.path.join(base, trans_out)
    return config
...
def process_transaction_data(config):
    ...
    if str(output_path).startswith("s3://"):
        ...
        # Save locally first to avoid R2 multipart chunking errors
        df_res_unit.to_parquet(local_tmp_path, engine='pyarrow', index=False)
        
        # Upload the whole, completed file to Cloudflare R2 cleanly
        fs.put(local_tmp_path, output_path)
        
        # Clean up the local temp file
        os.remove(local_tmp_path)
    else:
        # Fallback for local folder testing
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        df_res_unit.to_parquet(output_path, engine='pyarrow', index=False)
